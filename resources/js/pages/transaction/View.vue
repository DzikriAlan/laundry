              ...
              <div class="row">
                  <div class="col-md-6" v-if="transaction.status == 0">
                      ...
                      <div class="col-md-6" v-if="transaction.status == 0">
                        ...
                        <p v-if="isCustomerChange">Kembalian: Rp {{ formatToRupiah(customerChangeAmount)}}</p>
                  ...
                  <div class="col-md-6" v-if="transaction.payment">
                      ...
                      <table>
                          <tr>
                              ...
                              <td width="5%">:</td>
                              <td>Rp {{ formatToRupiah(transaction.payment.amount) }}</td>
                          </tr>
                          <tr>
                              <th>Kembalian </th>
                              <td>:</td>
                              <td>Rp {{ formatToRupiah(transaction.payment.customer_change) }}</td>
                          </tr>
                          <tr>
                          ...
                          <table class="table table-hover table-bordered">
                              ...
                              <tbody>
                                  ...
                                  <tr v-for="(row, index) in transaction.detail" :key="index">
                                      ...
                                      <td>Rp {{ formatToRupiah(row.price) }} / {{ row.product.unit_type }}</td>
                                      <td>Rp {{ formatToRupiah(row.subtotal) }}</td>
                                      <td>
...
<script>
  ...
  export default {
      ...
      computed: {
          ...
          formattedRupiah: {
              get() {
                return this.formatToRupiah(this.amount);
              },
              set(value) {
      ...
      methods: {
        ...mapActions('transaction', ['detailTransaction', 'payment', 'completeItem']), 
          ...
          updateRealValue(event) {
            this.transaction.price = this.formatToNumber(event.target.value);
          },
          makePayment() {
